...
# ╔═╡ 233071de-8cba-11f0-1524-6d70e8d6c89c
import RDatasets, DataFrames

# ╔═╡ e71c0ee6-78d7-4381-934b-1f30856739bd
begin
	df = RDatasets.dataset("datasets", "warpbreaks")
	first(df, 5) |> println
end

# ╔═╡ 0463424f-c73e-436c-812e-a3a424fc36b1
import Turing: @model, NUTS, sample, filldist, describe, logpdf

# ╔═╡ af980f0c-02e9-43a2-b502-7899e1c59a73
import CategoricalArrays: categorical, levelcode

# ╔═╡ 42a9d02b-a4c1-4856-88e9-561da2b12b51
begin
	df.cat = levelcode.(categorical(string.(df.Tension, "-", df.Wool)))
	df.logBreaks = log.(df.Breaks)
end

# ╔═╡ 7f0c3422-0e8a-48ef-b37e-a4d68dba0494
import Distributions: Normal, InverseGamma

# ╔═╡ 6e4ab5d6-f40e-420d-81fc-77fd21e863c8
@model function anova_t(X, y)
	num_cat = size(unique(X.cat), 1)
	num_obs = size(y, 1)
	
	μ ~ filldist(Normal(0.0, 10^3), num_cat)
	σ² ~ filldist(InverseGamma(1/2.0, 1.0/2.0), num_cat)
...
# ╔═╡ 10ee44da-ccbc-4fc1-b0c5-3695dbf57196
begin
	model_1 = anova_t(df[:, [:cat]], df.logBreaks)
	chains = sample(model_1, NUTS(), 5000)
end

# ╔═╡ f024b0ae-459c-44f9-91ec-1c3f9d9eda98
unique(df.cat)

# ╔═╡ a49dddb6-85f6-4e8f-81fc-928e72585df2
describe(chains)

# ╔═╡ 2a6ad60d-2a1a-465f-9a4e-31ce1afed0d2
log_likelihood(dat, params) = begin
	total_cat = size(unique(dat.cat), 1)
	
    μ = params[iter=1, var=[Symbol("μ[$i]") for i in 1:total_cat]]
	σ = sqrt.(params[iter=1, var=[Symbol("σ²[$i]") for i in 1:total_cat]])
...
begin
	...
	# Mean correction
	sigmasym = [Symbol("σ²[$i]") for i in 1:6]
	sigmavals = chains.value[iter=:, var=sigmasym, chain=1]
	corrected_variance = mean(sqrt.(sigmavals), dims=1) .^ 2
	parameters_mean = mean(chains.value[iter=:, var=:, chain=1], dims=1)
	
	for i = 1:6
		parameters_mean[iter=1, var=sigmasym[i]] = corrected_variance[i]
	end

	
	
	deviance_at_mean = -2 * log_likelihood(df,  parameters_mean)

	mean_deviance |> println
	mean_deviance - deviance_at_mean |> println
